using System.Collections;
using System.Collections.Generic;
using UnityEngine;

[CreateAssetMenu(fileName = "Wave Data", menuName = "2D Top-Down Rogue-Like/WaveData")]
public class WaveData : SpawnData
{
    [Header("Wave Data")]

    [Tooltip("If there are less than this number of enemies, keep spawning until count reached.")]
    [Min(0f)] public int startingCount = 0;

    [Tooltip("How many enemies can this wave spawn at maximum.")]
    [Min(1)] public uint totalSpawns = uint.MaxValue;

    [System.Flags] public enum ExitCondition
    {
        waveDuration = 1,
        reachedTotalSpawns = 2
    }

    [Tooltip("Set the things that can trigger the end of this wave.")]
    public ExitCondition exitCondition = (ExitCondition)1;

    [Tooltip("All enemies must be dead for the wave to advance.")]
    public bool mustKillAll = false;

    [HideInInspector]
    public uint spawnCount; //The number of enemies already spawned in this wave

    /*Returns an awway of prefabs that this wave can spawn.
     Takes an optional parameter of how many enemies are on the screen at the moment.*/
    public override GameObject[] GetSpawns(int totalEnemies = 0)
    {
        //Determine how many enemies to spawn.
        int count = Random.Range(spawnsPerTick.x, spawnsPerTick.y);

        /*If less than <minimumEnemies> on screen,
         set the count to be equal to the number of enemies to spawn
        to populate the screen until it has <minimumEnemies> within.*/
        if (totalEnemies + count < startingCount)
        {
            count = startingCount - totalEnemies;
        }

        //Generate result.
        GameObject[] result = new GameObject[count];
        for (int i = 0; i < count; i++) 
        {
            /*Randomly picks one of the possube spawns
             and inserts it into the results array.*/
            result[i] = possibleSpawnPrefab[Random.Range(0, possibleSpawnPrefab.Length)];
        }

        return result;
    }
}
